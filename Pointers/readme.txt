...
1. Data variable -> int x=10;   ->  2 bytes of memory located in stack position 0x3e935ff9b4
...
Another reason to use pointers is because programming languajes works with Stacks which means that information can be accessed one time 
the it is deleted because it works as stack Arithmetic so if you need that information for example the information on a single array if you dont wanna lost it
you need to use pointers because pointers store information on the heap and the positive thing with heap is that your information keeps as long as your code is running
and stack this thing doesn't works like that.
...
you use a pointer on a funcion but this information is needed only when the funcion is used when you dont need more the function
that pointer must be delete it because if you stop using that funcion you will have a memory leak because that information on the heap
any pointer is not referencing that so that information is loss and consuming space (no pointer means no memory use )
...
int *p; 
p=10; it does not work because we are assign a value to p that is a pointer that is poiting to nothing
...
1.2 p= int*0x5438; (gives directly a memory location to point)

1.3 p= new int[5] you can dynamically deallocate using the heap memory so you can create an array to initialize
...
reference doesnt consume any memory extra my parents house belongs to my mother and my father

you cant overname or change nickname to be part of another variable
